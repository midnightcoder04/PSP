...
- **AC-4 (WATUSI auto-count + linked rank slide)**: The WATUSI checklist exercise computes counts per group (W/A/T/U/S/I) live as the participant ticks items. The ranking slide for the six attitude types renders with the order pre-derived from those counts (descending count, then canonical WATUSI order W,A,T,U,S,I as tiebreaker). The displayed counts beneath each ranked item are **read-only**; editing a checklist tick instantly refreshes the **count badges**. **Re-derivation rule**: the prefilled order is computed *only* when no manual ranking response exists yet (`responses[ranking_id]` is null). Once the participant has saved a manual order — including the participant explicitly accepting the prefilled order via the "Save" action or by advancing the slide — subsequent checklist edits update the count badges but DO NOT clobber the saved order. The ranking can still be manually reordered (drag-and-drop) at any time; manual reorders persist. Both exercises live on **one logical slide** (single navigation step) so the participant sees cause-and-effect together.

- **AC-5 (Values floating widget)**: While entering values in the Shopping Spree table, a fixed-position widget shows **Spent: $X / Remaining: $Y** with `Y = 100,000 - X`. The widget uses these tokens for state: `--color-text-primary` while `0 < Y ≤ 100,000` (in progress), `--color-success` when `Y === 0` (perfectly allocated), and `--color-warning` when `Y < 0` (over-budget). The follow-on slide's **Proceed →** button is disabled unless `X === 100,000`.
...
- **AC-9 (Goal Setting refinements)**: The Goal Setting section adopts the same patterns it can reuse: structured multi-question text where applicable; numeric/picker inputs where 1–5 ratings appear; and the slide nav from AC-1/AC-2. No new exercise *kinds* invented here. **Testable assertion**: after the audit task completes, every exercise in section `goal-setting` either uses `structured-text`, `rating-picker`, `ranking`, `table`, `checkbox`, or `info`, OR is documented in `specs/003-slide-nav-ux-rework/goal-setting-audit.md` with a one-line justification for keeping the existing type. **No `text`-typed exercises with more than one implicit sub-question (numbered list inside the prompt) remain** in the Goal Setting section.

- **AC-10 (Testimonial submission)**: On the course-completion screen the participant sees a **Leave a testimonial** button. It opens a form: a required `content` textarea (≥ 50 chars, ≤ 1 500 chars), an optional integer rating 1–5, and a submit. Submitting inserts into a new `testimonials` table keyed by `(participant_id, session_id)`, with `session_id` resolved to the most recent active enrollment for the participant. Re-submitting updates the existing row (uniqueness on participant+session enforces "one testimonial per session per participant"). **Edge case**: If the participant has no active enrollment (a solo learner), the form opens but submission is blocked with the message *"No active session — please contact your facilitator."*
...
- **FR-011 (testimonials table)**: A new table `testimonials(id uuid pk, participant_id uuid fk, session_id uuid fk, content text not null, rating int null check (rating between 1 and 5), submitted_at timestamptz default now(), updated_at timestamptz default now(), unique(participant_id, session_id))`. The `session_id` FK uses `ON DELETE RESTRICT` so a session with any testimonial cannot be deleted via the data API; admin moderation that needs to remove a session must first archive/delete its testimonials with the service-role key.
...
- **SC-002**: WATUSI: ticking 7 W-items + 3 S-items results in W leading the prefilled ranking with count badge "7", S in position 2 with "3", and the remaining items ordered A, T, U, I (canonical WATUSI tie-break) each with count badge "0".
