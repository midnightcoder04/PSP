...
/** Fetch the top-8 goal labels from goal-priorities + life-goal-inventory. */
async function fetchDerivedGoalNames(
  participantId: string,
  ...
  if (!exercises || exercises.length < 2) return emptyNames()
  ...
  if (!gpResp?.order || !lgiResp?.rows) return emptyNames()
  ...
  const names = gpResp.order.slice(0, GOALS).map((id) => goalMap[id] ?? '')
  while (names.length < GOALS) names.push('')
  return names
}
...
export function CrossImpactMatrix({
  ...
  useEffect(() => {
    ...
    fetchDerivedGoalNames(participantId, sessionId).then((names) => {
